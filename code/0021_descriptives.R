...
## TABLE on imputed years with purges / packing
...
imputed_years %>% 
  ...
  cols_label(
    country = "Country ",
    year = "Year ",
    trust_share_high_imp_lastv = "Closest Imp.",
    trust_share_high_linear_imp = "Linear Imp.",
    trust_share_high_NAyears = "Missing Years",
    trust_share_high_NAchange = "Change b/w Observations"
  ) %>% 
...
writeLines(table_imputation, "results/tables/imputation.tex")
...
governmentdata_world %>% 
  ...
  coord_cartesian(clip = "off") +
  theme(axis.title.y.left = element_text(size = textsize/0.9,
                                         family = "Arial",
                                         hjust = 0.5))
...
data1 <- readRDS("data/analysis_jud_replace_cont.rds")
t.test(data1$ruth_populism, data1$jud_replace_cont)
t.test(data1$ruth_populism, data1$jud_replace_cont, var.equal = TRUE)


data2 <- readRDS("data/analysis_v2jupoatck.rds")
t.test(data2$ruth_populism, data2$v2jupoatck)
t.test(data2$ruth_populism, data2$v2jupoatck, var.equal = TRUE)

data1 %>% 
  full_join(data2) %>% 
  filter(!is.na(ruth_populism)) %>% 
  pivot_longer(cols = c(v2jupoatck, jud_replace_cont),
               names_to = "var",
               values_to = "val") %>% 
  mutate(var = if_else(var == "v2jupoatck", "Attacks", "Curbing & Packing")) %>% 
  ggplot(aes(y = var,
    x = val,
  ...
  annotate(geom = "text",
          ...
          vjust = -0.5) +
  labs(y = NULL,
      x = "Government attacks on judiciary",
      color = NULL,
      fill = NULL) +
  scale_color_manual(values = c(color_dark, color_colorful),
  ...
  scale_fill_manual(values = c(color_dark, color_colorful),
                    labels = c("Non-Populist", "Populist")) +
  scale_x_continuous(breaks = c(-3, 0, 3, 5),
                     limits = c(-3, 5.1),
                     expand = c(0,0)) +
